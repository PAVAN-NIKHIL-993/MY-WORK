...
- **File:** [`05-python-tools/serial_logger.py`](../../05-python-tools/serial_logger.py) (~110 lines) · **Module:** [python-tools](../modules/python-tools.md)
...
- **Errors:** all in [error-handling](../architecture/error-handling.md); no tracebacks for user/device faults (fixed E1/E2).
- **Perf:** I/O-bound; flush/line free at serial rates. **Security:** port bytes untrusted (replace-on-decode, CSV quoting); no shell.
- **Failure modes:** exit 3 (port busy/wrong/perms — `--list` first); garbage (baud mismatch — `baud_calc` first).
